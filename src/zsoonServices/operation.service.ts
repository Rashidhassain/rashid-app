...
export class operationServices {
  ...
  // GetParkwayPatientLogs(apiEndPoint, params) {
  //   const { page, limit, filter, download } = params;
  //   let url = `${apiEndPoint}PatientLogParkway/GetAllPatientLogs?`
  ...
  GetActivityLogs() {
    ...
    const options = { headers: headers };
    return this.http.get(
      environment.baseURL + 'ActivityLogs/GetAllActivityLogs',
      options
    ).pipe(
      retry(1),
      catchError(this.handleError)
  ...
  SendTestMail(value) {
    ...
    const options = { headers: headers };
    return this.http.get(environment.baseURL + 'ScheduledReports/ToTestMail?email=' + value, options);
  }
